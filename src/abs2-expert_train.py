...
def get_cwe_dataset_new(df,cwe,other_cwe_as_neg=False):
    cnt = 0
    for index, row in df.iterrows():
        if row['target'] == 1 and row['new_target'] == cwe:
...
def main():    
    ...
    if args.cwe != "binary":
        train_pd = get_cwe_dataset_new(train_pd, args.cwe,args.force_cwe)
        print(f'train_pd_all: {train_pd["target"].value_counts()}')
        
        val_pd = get_cwe_dataset_new(val_pd, args.cwe)
        print(f'val_pd_all: {val_pd["target"].value_counts()}')
        
        test_pd = get_cwe_dataset_new(test_pd, args.cwe)
