...
def trainer(train_pd, val_pd, test_pd, args):
    ...
    gamma = 1
    if args.ros:
        model_path = f"../models/output_{selected_model.split('/')[-1]}_seed{args.seed}/ros_ep{epochs}_bs{batch_size}_eval_f1_macro"
    else:
        model_path = f"../models/output_{selected_model.split('/')[-1]}_seed{args.seed}/focal_ep{epochs}_bs{batch_size}_eval_f1_macro_gamma{gamma}"
...
def parse_args():
    ...
    parser.add_argument(
        "--train_file",
        type=str,
        default="../data/train_cwe.parquet",
        help="train file",
    )
    parser.add_argument(
        "--test_file",
        type=str,
        default="../data/test_cwe.parquet",
        help="test file",
    )
    parser.add_argument(
        "--val_file",
        type=str,
        default="../data/val_cwe.parquet",
        help="val file",
    )
